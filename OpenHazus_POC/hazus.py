        ...
        for dgp in DGrids:
        	...
        	with open(UDFOrig, newline='') as csvfile:
        		...
        		for row in file:
        			...
        			depth = rastervalue - FFHeight if rastervalue is not 0 else 0   # Must mind the empty (null) case where the UDF has no Raster values
        			...
        			if CoastalZoneSupplied:
        				CoastalZoneCode = getValue(flC) # Only acquire if a Coastal Zone is defined for that UDF
        			...
        			if depth is 0 or depth < -500:	# Depending on Depth Grid format, the Extract_2_Point returns Null or -9999
